...
from api.util import get_or_404, generate_settings_file, zip_files
...
@app.get("/api/download/{user_id}/", response_class=FileResponse)
def download_post_editor(request: Request, user_id: int):
    settings_file = generate_settings_file(request.base_url, user_id)
    post_edit_zip = zip_files([settings_file, os.path.join(DIR_DOWNLOAD, "PostEditor.exe")], user_id)
    return FileResponse(post_edit_zip, filename="PostEditor.zip")
